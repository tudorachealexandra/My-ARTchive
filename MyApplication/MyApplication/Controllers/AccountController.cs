    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> RegisterUser(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                System.Diagnostics.Debug.WriteLine("inainte de if");
                user.ImagePath = "~/Images/iconUser.jpg";

                /*
                if (user.Image != null)
                {
                    System.Diagnostics.Debug.WriteLine("in if");
                    var filename = string.Concat(RandomString(10),user.Image.FileName);
                    var path = string.Concat("~/Images/", filename);
                    user.Image.SaveAs(Server.MapPath("~/Images/") + filename);
                    user.ImagePath = path;
                }
                */
        ...
        // POST: /Account/RegisterEditor
        [HttpPost]
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterEditor(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.UserName, Email = model.Email, ImagePath = model.ImagePath};
                System.Diagnostics.Debug.WriteLine("inainte de if");
                user.ImagePath = "~/Images/iconUser.jpg";
               /*
               if (model.Image != null)
               {
                   System.Diagnostics.Debug.WriteLine("in if");
                   var filename = string.Concat(RandomString(10), model.Image.FileName);
                   var path = string.Concat("~/Images/", filename);
                   user.Image.SaveAs(Server.MapPath("~/Images/") + filename);
                   user.ImagePath = path;
               }
